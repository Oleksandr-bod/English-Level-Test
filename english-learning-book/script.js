...
part of their lives at present.`;
    ...
    // ---------- ФУНКЦІЯ ДЛЯ ОТРИМАННЯ СТОРІНКИ З URL ----------
    ...
    // ---------- ЛОКАЛЬНИЙ СЛОВНИК ДЛЯ ВСІХ ПЕРЕКЛАДІВ ----------
    // Тут можна додавати нові слова в міру необхідності.
    // Для повноцінної роботи без API словник має містити всі слова з книги.


    // ---------- ФУНКЦІЇ ПЕРЕКЛАДУ (без API) ----------
    function normalizeWordForLookup(word) {
        // Замінюємо «розумні» апострофи на звичайні для узгодженості ключів
...
const hardcodedTranslations = {
    ...
    async function fetchTranslation(word) {
        ...
        // 1. Перевіряємо в кеші сесії
        if (translationCache.has(normalized)) {
            return translationCache.get(normalized);
        }

        // 2. Перевіряємо в localStorage
        const cached = localStorage.getItem(`translation_${normalized}`);
        if (cached) {
            translationCache.set(normalized, cached);
            return cached;
        }

        // 3. Шукаємо в локальному словнику
        let translation = hardcodedTranslations[normalized];

        // 4. Якщо не знайдено, пробуємо розгорнути скорочення
        if (!translation) {
            const expanded = expandContraction(word);
            if (expanded) {
                const expandedNorm = normalizeWordForLookup(expanded);
                translation = hardcodedTranslations[expandedNorm];
            }
        }

        // 5. Якщо все ще немає – повертаємо повідомлення
        if (!translation) {
        ...
        localStorage.setItem(`translation_${normalized}`, translation);

        return translation;
    }

    async function showTranslation(event, wordElement, originalWord) {
        ...
        tooltip.innerHTML = `<span class="loading-indicator"></span> переклад...`;
        tooltip.style.opacity = '1';
        tooltip.style.left = (rect.left + scrollX + rect.width / 2) + 'px';
        tooltip.style.top = (rect.top + scrollY - 45) + 'px';
        tooltip.style.transform = 'translateX(-50%)';
        tooltip.style.whiteSpace = 'nowrap';

        const translation = await fetchTranslation(originalWord);

        if (currentTooltipWord !== wordElement) return;

        tooltip.innerHTML = `${originalWord} → ${translation}`;
        tooltip.style.whiteSpace = 'nowrap';
        ...
        tooltip.style.left = (newRect.left + scrollX + newRect.width / 2) + 'px';
        tooltip.style.top = (newRect.top + scrollY - 45) + 'px';
    ...
    function onWordClick(event) {
        const wordSpan = event.currentTarget;
        const originalWord = wordSpan.getAttribute('data-word') || wordSpan.textContent.trim();

        if (currentTooltipWord === wordSpan) {
            hideTooltip();
            setTimeout(() => {
                currentTooltipWord = wordSpan;
                showTranslation(event, wordSpan, originalWord);
            }, 50);
            return;
        }
    ...
    function buildTextWithSpans(text) {
        container.innerHTML = '';
        ...
        lines.forEach(line => {
            ...
            const tokens = line.split(/([A-Za-z\u2019']+)/);
    ...
    function updatePaginationButtons() {
        ...
        pageIndicator.textContent = `Сторінка ${currentPage + 1} / ${totalPages}`;
    ...
    function goToPage(pageIndex) {
        ...
        buildTextWithSpans(pages[currentPage]);
    ...
    window.addEventListener('resize', function() {
        if (currentTooltipWord) {
            const rect = currentTooltipWord.getBoundingClientRect();
    ...
    // ---------- Функція для отримання унікальних слів ----------
    function getUniqueWordsFromText(text) {
        ...
        while ((match = wordRegex.exec(text)) !== null) {
            ...
            if (!wordsSet.has(lower)) {
                wordsSet.add(lower);
                uniqueWords.push(original);
            }
        }
        return uniqueWords;
    }

    // ---------- Кнопка для переходу на сторінку вивчення слів ----------
    const learnBtn = document.getElementById('learn-page-words');
    if (learnBtn) {
        learnBtn.addEventListener('click', () => {
            const currentText = pages[currentPage];
            const words = getUniqueWordsFromText(currentText);
            if (words.length < 4) {
                alert('На цій сторінці замало слів для гри (потрібно хоча б 4).');
                return;
            }
            sessionStorage.setItem('learningWords', JSON.stringify(words));
            sessionStorage.setItem('bookPage', window.location.href.split('?')[0]);
            sessionStorage.setItem('bookCurrentPage', currentPage + 1);
            window.location.href = 'englishLearningWords/index.html';
        });
    }
})();
